...
local function ensureTable()
    MySQL.query.await([[CREATE TABLE IF NOT EXISTS `bl_water_stations` (
      ...
      `liquid_type` varchar(40) NOT NULL DEFAULT 'dirty_water',
...
local function loadAll()
    ...
    for _, r in ipairs(rows) do
        Tanks[r.id] = { id=r.id, owner=r.owner, tankType=r.tank_type, coords=vec3(r.x,r.y,r.z), heading=r.heading, health=r.health, liquidType=r.liquid_type or 'dirty_water', amount=tonumber(r.amount) or 0.0, updatedAt=r.updated_at or nowUnix(), fixed=(r.is_fixed==1)}
    end
...
lib.callback.register('liquid_stations:server:getTank', function(_, tankId)
    return Tanks[tankId]
end)
...
lib.callback.register('liquid_stations:server:placeStation', function(source, tankType)
    ...
    if (exports.ox_inventory:GetItem(source, cfg.item, nil, true) or 0) < 1 then return { ok=false, err='missing_item' } end
    ...
    Tanks[id] = { id=id, owner=xPlayer.identifier, tankType=tankType, coords=vec3(p.x,p.y,p.z), heading=GetEntityHeading(GetPlayerPed(source)), health=cfg.maxHealth, liquidType=cfg.type, amount=0.0, updatedAt=nowUnix(), fixed=false }
...
RegisterNetEvent('liquid_stations:server:itemAction', function(id, item, mode)
    ...
    if mode == 'fill' then
        local map = (Config.FillItems[t.liquidType] or {})[item]; if not map then return end
    ...
    elseif mode == 'collect' then
        local map = (Config.CollectItems[t.liquidType] or {})[item]; if not map then return end
    ...
    elseif mode == 'drain' then
        local drain = math.min(t.amount, tonumber(Config.DrainPerAction or 5.0) or 5.0)
        t.amount = clamp(t.amount - drain, 0.0, cfg.capacity)
        TriggerClientEvent('ox_lib:notify', src, { title='Tank System', description=('Drained %.1fL'):format(drain), type='success' })
    else
        return
    end
...
RegisterNetEvent('liquid_stations:server:collectFromSource', function(sourceId)
    local src = source
    local sourceCfg
    for _, ws in ipairs(WaterSources or {}) do if ws.id == sourceId then sourceCfg = ws break end end
    if not sourceCfg then return end
    local p = GetEntityCoords(GetPlayerPed(src))
    if #(p - sourceCfg.coords) > (sourceCfg.radius + 2.0) then return end
    local rule = (Config.FillItems.dirty_water or {}).empty_waterbottle; if not rule then return end
    if (exports.ox_inventory:GetItem(src, 'empty_waterbottle', nil, true) or 0) < (rule.remove or 1) then return end
...
RegisterNetEvent('liquid_stations:server:boilTankWater', function(id)
    local src = source
    local t = Tanks[id]; if not t then return end
    local cfg = Config.Containers[t.tankType]; if not cfg or not cfg.canBoil then return end
    if t.liquidType ~= 'dirty_water' then return end
    local liters = tonumber(t.amount) or 0.0; if liters <= 0 then return end
    ...
    for item, rules in pairs(Config.BurnableItems or {}) do
        ...
        if (exports.ox_inventory:GetItem(src, item, nil, true) or 0) >= need then
            ...
            burnedItem = { name=item, count=need }
            break
        end
    end
    if not burnedItem then
        TriggerClientEvent('ox_lib:notify', src, { title='Tank System', description=('Need fuel x%s'):format(requiredFuel), type='error' })
        return
    end

    t.liquidType = 'water'
    t.updatedAt = nowUnix(); persistTank(t); broadcast()
    TriggerClientEvent('ox_lib:notify', src, { title='Tank System', description=('Boiled with %s x%s'):format(burnedItem.name, burnedItem.count), type='success' })
end)
...
RegisterNetEvent('liquid_stations:server:dismantleTank', function(id)
    ...
    Tanks[id] = nil; MySQL.update.await('DELETE FROM bl_water_stations WHERE id = ?', { id }); broadcast()
end)

CreateThread(function()
    ensureTable(); loadAll(); broadcast()
    while true do
        ...
        for id, t in pairs(Tanks) do
            ...
            if cfg then
                ...
                local prev = t.updatedAt or 0; t.updatedAt = nowUnix()
                local del = cutoff and not t.fixed and (prev < cutoff) or false
                if del then
                    if Config.DELETE_OPTIONS.DeleteOnlyIfEmpty and t.amount > 0 then del = false end
                    if Config.DELETE_OPTIONS.DeleteOnlyIfBroken and t.health > 0 then del = false end
                end
                if del then Tanks[id] = nil; MySQL.update.await('DELETE FROM bl_water_stations WHERE id = ?', { id }) else persistTank(t) end
            end
        end
        broadcast()
    end
